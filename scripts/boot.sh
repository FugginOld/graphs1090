...
# adsb-graphs is mostly used on RPis, avoiding frequent disk writes is preferred
...
IHTML=/usr/share/adsb-graphs/html/index.html
...
if [[ $all_large == "yes" ]]; then
    if grep -qs -e 'flex: 50%; // all_large' /usr/share/adsb-graphs/html/portal.css; then
        sed -i -e 's?flex: 50%; // all_large?flex: 100%; // all_large?' /usr/share/adsb-graphs/html/portal.css
        sed -i -e 's?display: flex; // all_large2?display: inline; // all_large2?' /usr/share/adsb-graphs/html/portal.css
    fi
else
    if ! grep -qs -e 'flex: 50%; // all_large' /usr/share/adsb-graphs/html/portal.css; then
        sed -i -e 's?flex: 100%; // all_large?flex: 50%; // all_large?' /usr/share/adsb-graphs/html/portal.css
        sed -i -e 's?display: inline; // all_large2?display: flex; // all_large2?' /usr/share/adsb-graphs/html/portal.css
    fi
fi
...
# disable this for the moment
	...
	#/usr/share/adsb-graphs/rem_rra.sh /var/lib/collectd/rrd/localhost/
#fi
...
do
	/usr/share/adsb-graphs/adsb-graphs.sh $i $1 &>/dev/null &
    if ! wait; then
        echo "boot.sh(adsb-graphs): early exit"
        exit 0
    fi
done
